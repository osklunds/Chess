...
newGameState = GameState {board = defaultBoard, turn = White, captured = []}
...
applyMove move (GameState {board, turn, captured}) = (nextGameState, result)
  ...
  where
    ...
    nextGameState =
        GameState {board = board', turn = opponent, captured = captured'}
    -- Determining result
    ...
    result =
        case (oppCanMoveUnthreatened, oppIsThreatened) of
            (True, True) -> Check
            (True, False) -> Normal
