        ...
        Just move -> do
            case validateMove move gameState of
                ...
                Ok -> do
                    ...
                    printBoard gameState'
                    case result of
                        Normal -> computerTurn gameState'
                        Check -> do
                            putStrLn "Computer is checked"
                            computerTurn gameState'
                        Checkmate -> putStrLn "Checkmate! You win"
                        Draw -> putStrLn "Draw!"
...
computerTurn gameState = do
    ...
    case result of
        Normal -> playerTurn gameState'
        Check -> do
            putStrLn "You're checked"
            playerTurn gameState'
        Checkmate -> putStrLn "Checkmate! Computer wins"
        Draw -> putStrLn "Draw!"
...
printBoardWithMove (start, dest) =
    putStrLn . (showBoardWithMarkers [start, dest]) . board
...
showRow posList board row =
  ...
  where
    ...
    rowString =
        P.concat
            [ squareChar ++ betweenChar
            | col <- [0 .. 7]
            , let squareChar = squareCharForPosition (row, col) board
            , let betweenChar = betweenCharForPosition posList (row, col)
            ]
