...
movesForColor color board =
    concat
        [movesFromPos (row, col) color board | row <- [0 .. 7], col <- [0 .. 7]]
...
movesFromDirs ::
       [(Int, Int)] -> (Int, Int) -> Board -> [((Int, Int), (Int, Int))]
...
movesFromColorAndDiff ::
       Color -> (Int, Int) -> (Int, Int) -> Board -> [((Int, Int), (Int, Int))]
...
knightMoves start board = filter hasValidDest moves
  where
    ...
    moves = map (\diff -> (start, start `tupleAdd` diff)) knightDiffs
    hasValidDest (_start, dest) =
        let atDest = getB dest board
         in isWithinBoard dest && not (isColor color atDest)
...
pawnMoves start board = [(start, dest) | dest <- dests]
  where
    ...
    dests =
        [left | isWithinBoard left && isOtherColor color atLeft] ++
        [right | isWithinBoard right && isOtherColor color atRight] ++
        [forward | isWithinBoard forward && isEmpty atForward] ++
        [ doubleForward
        | isWithinBoard doubleForward &&
              isEmpty atDoubleForward && isEmpty atForward && isAtInitial
        ]
