...
movesAreSubsetOfMoves kind1 kind2 board pos = movesKind1 `isSubsetOf` movesKind2
  where
...
swapColors board =
    foldl swapColorPos board [(row, col) | row <- [0 .. 7], col <- [0 .. 7]]
...
mirrorBoard board =
    foldl mirrorPos board [(row, col) | row <- [0 .. 3], col <- [0 .. 7]]

mirrorPos :: Board -> (Int, Int) -> Board
mirrorPos board pos@(row, col) =
    setB pos atMirroredPos $ setB mirroredPos atPos $ board
  where
    mirroredRow = 7 - row
...
pieceOnlyMoves ::
       Kind -> (((Int, Int), (Int, Int)) -> Bool) -> Board -> (Int, Int) -> Bool
...
isMoveHorizontalOrVertical ((rowS, colS), (rowD, colD)) =
    rowS - rowD == 0 || colS - colD == 0

isMoveDiagonal :: ((Int, Int), (Int, Int)) -> Bool
isMoveDiagonal ((rowS, colS), (rowD, colD)) =
    abs (rowS - rowD) == abs (colS - colD)
...
movesIfCan getDir kind maxLength board start dir length =
    ...
    emptyBetweenStartAndDest board' move ==> move `elem` moves
  where
...
prop_pawnSingleForward board pos =
    isWithinBoard dest ==> (move `elem` moves) == isEmpty atDest
  where
...
prop_pawnDoubleForward board pos =
    isWithinBoard dest ==> (move `elem` moves) ==
    (isEmpty atBetween && isEmpty atDest && startRow == 1)
  where
    (board', start, moves) = placePieceAndGetMoves board pos Pawn
...
prop_pawnMovesRight board pos =
    isWithinBoard dest ==> (move `elem` moves) == isOtherColor Black atDest
  where
    (board', start, moves) = placePieceAndGetMoves board pos Pawn
...
prop_pawnNoOtherMove board pos =
    counterexample errorString (moves `isSubsetOf` legalMoves)
  where
    (board', start, moves) = placePieceAndGetMoves board pos Pawn
...
isMoveLShaped ((rowS, colS), (rowD, colD)) =
    rowDiff `elem` [1, 2] && colDiff `elem` [1, 2] && rowDiff + colDiff == 3
  where
...
prop_knightMovesIfCan board pos rowDiff colDiff =
    isWithinBoard dest &&
    isMoveLShaped move && not (isColor Black atDest) ==> move `elem` moves
  where
